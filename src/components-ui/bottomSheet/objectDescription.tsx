...
const Comment = ({ comment }: { comment: CommentData | ReplyData }) => {
    ...
    return (<>
        <div className={`row top-border ${!isComment && "ps-3"}`}>
                ...
                <div className="row">
                    <div className="col-12 a4-comments__action-bar-container">
                        <div className="rating">
                            <button className={`rating-button rating-up ${comment.isLiked && "liked"}`}><i className="far fa-thumbs-up"></i>{comment.likes}</button>
                            <button className={`rating-button rating-down ${comment.isDisliked && "disliked"}`}><i className="far fa-thumbs-down"></i>{comment.dislikes}</button>
                        </div>
                        {isComment &&
                            <div className="a4-comments__action-bar">
                                <button className="btn btn--no-border a4-comments__action-bar__btn" type="button" onClick={() => setIsShowingReplies((prev) => !prev)}>
                                    {isShowingReplies
                                        ...
                                        : <><i className="far fa-comment"></i>{comment.replies.length} Reply</>
                                    }
                                </button>
                            </div>
                        }
        ...
        >
            <div className="minh-100 d-flex flex-column" style={{ fontSize: "0.8rem" }}>
                ...
                <div className="row top-border">
                    <div className="col-12 a4-comments__action-bar-container">
                        <div className="rating">
                            <button className={`rating-button rating-up ${variant.isLiked && "liked"}`}>
                                <i className="far fa-thumbs-up"></i>{variant.likes}
                            </button>
                            <button className={`rating-button rating-down ${variant.isDisliked && "disliked"}`}>
                ...
                {object.comments.map((comment) => <Comment key={comment.id} comment={comment} />)}
            </div>
        </BottomSheet >
    );
};
