...
const GeolocationPage = () => {
    ...
    useEffect(() => {
        const onSuccess = (position: GeolocationPosition) => {
            ...
            if (geoPosition.coords) {
                ...
                // TESTING: Uncomment to use GPS data instead of dummy location
    ...
    useEffect(() => {
        ...
        setCompassHeadingHistory(prev => {
            ...
            // Check if all values in history are within a small range
            // const minValue = Math.min(...updatedHistory);
            // const maxValue = Math.max(...updatedHistory);
    ...
    // Add scene center gizmo
    // const gizmo = new THREE.AxesHelper(0.25); // 1 unit size
    // sceneRef.current.add(gizmo);
    // function normalizeYawClockwise(yaw: number): number {
    //     return (360 - ((yaw + 360) % 360)) % 360;
    // }
    // const normalizedPhoneYaw = normalizeYawClockwise(referenceCompassHeading.phoneYaw);

    const groupRotationEulerY = (referenceCompassHeading.phoneYaw - referenceCompassHeading.heading + 540) % 360;;// (referenceCompassHeading.phoneYaw - referenceCompassHeading.heading + 540) % 360;
